...
void create_test(void) {
	...
	reo_offset_t symbol_name = reo_string_add(&file, "test");

	reo_embed_add(&file, string, data, sizeof(data));
	reo_symbol_add(&file, symbol_name, weird_address, 8, (reo_symbol_type_t) {false, false, false, REO_LOCATION_CODE, 0});
	reo_relocation_add(&file, string, symbol_name, 0);
